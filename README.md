...
**📜 v4.7.5 — صياغة صادقة + أولوية تعليمات اللعبة**
- الوعد المطلق «المحادثة الجديدة لازم تلعب بنفس الكفاءة بالضبط» اتبدّل في الدليل والأداة بـ: «تسترجع نفس البرنامج والمعايرة؛ الأداء والنتيجة يحتاجان تحققًا حيًّا» (ملاحظة قبل أول حركة وبعدها + قراءة النتيجة بالـ OCR، وأي اختلاف = إعادة فحص العنصر وتحديث الـ playbook).
- **قاعدة الأولوية (PRECEDENCE)** في رأس قسم 6 وداخل بلوك ALGORITHM: تعليمات اللعبة المحددة (الخوارزمية/الإجراء/المعايرة/الحيل) **تغلب** أي إرشاد عام متعارض في قسمي 6 و7 — مثال: لو البرنامج المحفوظ بيقرأ الشاشة بـ capture_screen منفصلة + مسح 3×3 أو OCR منفصل، ينفّذها رغم إن الافتراضي يفضّل play/play_frame. الـ MISTAKES فقط مطلقة.
- اختبارات e2e-v473 49/49 ✅.

**🧮 v4.7.4 — الخوارزمية الكاملة تنتقل مع الخبرة (مش وصف بس)**
- `playbook.algorithm {lang, description, code}` (حتى 24KB) + `playbook.calibration {screenW, screenH, offsets, cell…}`: الـ AI **ملزم** يحفظ كود المخطِّط/دالة التقييم اللي استخدمها فعلًا (أي نوع لعبة: puzzle planner، aim math للشوتر، مقيّم يد للكوتشينة، توقيتات الريسنج، build order للمزارع، افتتاحيات الشطرنج…) — وبيتحقن **نصًا كاملًا** في bootstrap المحادثة الجديدة بين علامتي `ALGORITHM START/END` مع أرقام المعايرة. قاعدة 13 في الدليل: لو فيه ALGORITHM في الـ playbook → استخدمه كما هو ولا تصمّم واحد جديد.
- إصلاح 3 فروق اكتشفها مساعد المحادثة الأخرى: (1) مقاس الشاشة — التطبيق 4.1.7 بيرسل مقاس الشاشة **الحقيقي** (720×1600 مع شريط التنقل) بدل نافذة التطبيق (1448)، والدليل بيوضّح إن مقاس الـ screenshot هو المرجع؛ (2) **تعويض السحب** (~95px) ومسح 3×3 بقوا محفوظين في calibration/tricks مع كود الـ planner الكامل لـ THNDR؛ (3) معرّف الجهاز في الكتاب العام بقى `my-phone`.
- دمج البروفايلات عند rename بقى **حقلًا بحقل** (controls/colors/regions/settings/strategies/bestScore/lastReport) — ماينفعش يضيع هندسة اللعبة تاني (كان اندماج سابق مسح الألوان/المناطق؛ اتصلّح واتعاد إدخالها).
- `/setup` بيعرض المعايرة والخوارزمية الكاملة (قابلة للطي). اختبارات e2e-v473 47/47 ✅.

**🧠 v4.7.3 — نقل الخبرة بين المحادثات + تغيير اسم الجهاز بدون فقد الذاكرة**
- **`playbook` (أداة #84) — كتاب خبرة لكل لعبة يكتبه الـ AI بنفسه**: overview، strategy (قواعد مرتّبة)، procedure (خطوات الحركة الواحدة)، tricks (اللي نجح بإحداثياته)، mistakes (ممنوع تكرارها)، screens (تمييز القوائم/الخسارة)، facts، skill 1-5. الفرق: البروفايل = *فين* الحاجات، session_report = *إيه اللي حصل*، playbook = *إزاي تلعب كويس*. بيتحقن **نصًا** في bootstrap أي محادثة جديدة (قسم 0 للعبة المفتوحة، 5g لباقي الألعاب، وكتاب عام `app:"*"` لمهارات كل الألعاب) — فالمحادثة الجديدة تلعب **بنفس المستوى من أول حركة** لأي لعبة مهما كان نوعها.
- **قاعدة 12 في الدليل**: تحديث الـ playbook إلزامي بعد أول ~10 حركات ناجحة، عند أي فشل، وقبل session_report. `play_loop` بيحدّثه أوتوماتيك (القواعد اللي نجحت → tricks، اللي ضرّت → mistakes).
- الـ playbooks ظاهرة في `/setup` (ذاكرة الـ AI → 🧠 كتاب الخبرة) مع حذف، ومتضمّنة في export/import.
- **تغيير Device ID مع نقل كل شيء**: `POST /api/admin/devices/:id/rename {newId}` (زرار «تغيير ID» في /setup) ينقل البروفايلات+الاستراتيجيات، الجلسات، الملاحظات، الماكروهات، الشاشات، الـ playbooks، التسمية، وكل توكنات الجهاز — ويقول للموبايل (لو متصل) يحفظ الاسم الجديد ويعيد الاتصال (أمر `set_device_id`، تطبيق 4.1.6). مفيد كمان بعد إعادة التثبيت (الـ id الافتراضي بيتغيّر → غيّر القديم للجديد وترجع الذاكرة).
- `./phone.sh playbook [app] | playbook merge '<json>' [app] | playbook delete`. اختبارات `tests/e2e-v473.sh` 41/41، وكل المجموعات 20/20 ✅.

**🕹️ Android 4.1.5 — إصلاح joystick + إطلاق نار**: الجويستيك بقى ضغطة واحدة متصلة (اضغط→ادفع→استمر) بدل قطع قابلة للإلغاء، وأي gesture أثناء أصابع مضغوطة يحمل استمرارها في نفس الإيماءة → اختفى `joystick hold: gesture cancelled` و`shot 3/3 failed`.
...
- **Hybrid**: قاعدة ممكن تحمل `reflex:{rules, timeoutMs}` بدل `do` → المرحلة دي تتسلّم لمحرك الـ 50ms على الجهاز (مثلًا: عدو ظاهر → aim_found + fire على الموبايل مباشرة).
- **session_report أوتوماتي** بعد كل تشغيل (النتيجة + النصائح) — لو المحادثة اتقطعت ما يتضيع شيء. اختبارات **894/894** ✅.
...
## v4.8 — بوت الدومينو All Fives (أمريكاني) على الهاتف نفسه · Android 4.2.0 · 85 أداة
اللعبة: **Domino** داخل `com.big.ludocafe`، وضع «أمريكاني» 1×1 بمؤقّت 10 ثوانٍ للدور. البوت يعمل **بالكامل على الهاتف** (بدون رحلة شبكة في كل حركة) حتى يلحق المهلة:

| طبقة | ملف | ماذا تفعل |
|---|---|---|
| رؤية | `android/.../domino/DominoVision.kt` | تقرأ لقطة الشاشة (أي دقة؛ هندسة مرجعية 1600×720): أحجار الإيد (مركز، نقاط أعلى/أسفل، رمادي = غير قابل للعب، أصفر = مختار)، أحجار الطاولة عبر خط الفاصل الرفيع لكل حجر (أفقي/عمودي + النقاط)، دور اللاعب من تلوين قرص الأفاتار بالعدّاد |
| قواعد + ذكاء | `android/.../domino/AllFives.kt` | نموذج الطاولة (spinner، جانبَيه، الذراعان U/D، الدبل في الطرف يُحسب ×2، ذراع غير ملعوب = 0)، الحركات القانونية، النقاط (مجموع الأطراف ÷5)، وتقييم كل حركة: نقاط فورية − أفضل رد متوقع للخصم (توزيع منتظم على الأحجار غير المرئية) + احتمال سدّ الخصم + مرونة إيدك + التخلّص من الأحجار الثقيلة + مكافأة «دومينو». 11 اختبار قواعد أخضر |
| الحلقة | `android/.../domino/DominoBot.kt` | كل ~300ms: لقطة → تحليل → لو دورك وقراءتان متطابقتان → بناء الطاولة هندسيًا (يختار الحجر الأقصى في كل خط وأين يُلقى الحجر التالي) → أفضل خطة → **سحب الحجر من الإيد إلى نقطة الإسقاط بالظبط** → تحقّق (قلّت الإيد/زادت الطاولة) وإلا يجرّب المرشّح التالي. لا حجر قابل للعب (كلها رمادي) → يضغط الرصّة للسحب |
| الإجراء | `domino_bot` (Action + أداة) | `op: start` (startDelayMs افتراضي 3500 لتعود للعبة) · `stop` · `status` (moves/fails/log) · `analyze` (إطار واحد: إيد/طاولة/أطراف/خطط مرتّبة لتصحيح الرؤية) |
| الواجهة | `/setup/<ADMIN_TOKEN>` → كارت «🎲 بوت الدومينو» | اختيار الجهاز، تأخير البدء 3/3.5/4/6 ث، زر **ابدأ** مع عدّ تنازلي، **إيقاف**، **تحليل الشاشة الآن**، ولوج حي كل 1.5 ث |

القواعد المطبَّقة (من صفحات «القواعد» داخل اللعبة): 7 أحجار لكل لاعب، الافتتاح بدبل، مطابقة أحد الأطراف، النقاط عندما يكون مجموع الأطراف من مضاعفات 5، السحب حتى حجرين ثم «مرر»، وعند القفل تُخصم نقاط الأحجار المتبقية ÷5.

استخدام: افتح اللعبة وادخل جولة → افتح الكارت في `/setup` → **ابدأ** → ارجع للعبة قبل انتهاء العدّ. الـ AI في محادثة جديدة يعرف الأداة من bootstrap (§5j) ويبدأها بدل اللعب حجرًا حجرًا.
