...
## 7a. HOW TO PLAY LIVE — you are the player (v4.1: play + react_script)
Two tools make you fast; everything else is for setup:
...
### 7.shooter — Free Fire, PUBG, CoD Mobile, Brawl Stars, any twin-stick / FPS / TPS  (Android app v4.1+)
  ...
  Reactive phase (fights): react_script {rules:[{name:"track",when:[{type:"color_present",color:"@enemy",minSize:8,maxSize:140}],then:[{op:"aim_found",x:"@crosshair",y:"@crosshair",dx:"@look",holdMs:"@look" ,distance:ratio,count:180,dy:-10}],cooldownMs:70},{name:"fire",priority:1,when:[{type:"color_present",color:"@ring",region:"@reticle",minCount:30}],then:[{op:"fire",at:"@fire",holdMs:300}],cooldownMs:60}], stopRules:[{type:"text_present",text:"DEFEAT"}], timeoutMs:10000}. @ring = the game's own red crosshair colour → fires only when the GAME confirms the target.
